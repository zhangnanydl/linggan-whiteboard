...
const loadToolCycle = () => {
  try { const value=JSON.parse(localStorage.getItem(TOOL_CYCLE_STORAGE_KEY)); if(value?.mode==='custom'&&Array.isArray(value.tools)&&value.tools.length>=3&&value.tools.length<=5)return value; } catch { /* use all tools */ }
  return {mode:'all',tools:['pen','eraser','text']};
};
...
const wrapShapeText = el => {
  if(!el.text)return{lines:[],size:18,lineHeight:23};
  const usableWidth=(el.type==='ellipse'?el.w*.72:el.w)-24,usableHeight=(el.type==='ellipse'?el.h*.72:el.h)-14;
  for(let size=Math.min(26,Math.max(16,Math.min(el.w*.18,el.h*.34)));size>=14;size-=1){
    const maxChars=Math.max(1,Math.floor(usableWidth/(size*.9))),lineHeight=size*1.28,maxLines=Math.max(1,Math.floor(usableHeight/lineHeight));
    const lines=el.text.split('\n').flatMap(line=>line.match(new RegExp(`.{1,${maxChars}}`,'g'))||['']);
    if(lines.length<=maxLines)return{lines,size,lineHeight};
    if(size===14){const visible=lines.slice(0,maxLines);if(lines.length>maxLines)visible[maxLines-1]=`${visible[maxLines-1].slice(0,-1)}…`;return{lines:visible,size,lineHeight};}
  }
  return{lines:[el.text],size:14,lineHeight:18};
};

function ShapeLabel({el}){
  const {lines,size,lineHeight}=wrapShapeText(el);if(!lines.length)return null;
  const x=el.x+el.w/2,startY=el.y+el.h/2-(lines.length-1)*lineHeight/2+size*.35;
  return <text className="shape-label" x={x} y={startY} textAnchor="middle" fill={el.textColor||el.color} fontSize={size} fontFamily="Inter,'Microsoft YaHei',sans-serif">{lines.map((line,index)=><tspan key={`${line}-${index}`} x={x} dy={index?lineHeight:0}>{line}</tspan>)}</text>;
}

function InlineTextEditor({editor,style,onCommit,onCancel}){
  const inputRef=useRef(null),finished=useRef(false);
  useEffect(()=>{const input=inputRef.current;if(!input)return;input.focus();input.select();},[]);
  const finish=()=>{if(finished.current)return;finished.current=true;onCommit(inputRef.current?.value??editor.value);};
  const cancel=()=>{finished.current=true;onCancel();};
  return <div className={`inline-text-editor ${editor.kind==='shape'?'inside-shape':'free-text'}`} style={style} onPointerDown={event=>event.stopPropagation()}><textarea ref={inputRef} aria-label={editor.kind==='shape'?'图形内文字':'画布文字'} defaultValue={editor.value} placeholder={editor.kind==='shape'?'直接输入图形文字':'直接输入文字'} onBlur={finish} onKeyDown={event=>{event.stopPropagation();if(event.key==='Escape'){event.preventDefault();cancel();}else if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)){event.preventDefault();finish();}}}/><small>{editor.kind==='shape'?'自动居中与换行':'Ctrl+Enter 完成 · Esc 取消'}</small></div>;
}
...
function Whiteboard(){
  ...
  const [tool,setTool]=useState('select'); const [color,setColor]=useState('#175cd3'); const [strokeWidth,setStrokeWidth]=useState(4); const [eraserSize,setEraserSize]=useState(28);
  ...
  const svgRef=useRef(null),viewportRef=useRef(null),gesture=useRef(null),keyboardState=useRef(null),saveTimer=useRef(null),dockTimer=useRef(null),dockHovered=useRef(false),lastWheelSwitch=useRef(0);
  ...
  const startInlineEditor=p=>{
    ...
    if(shape){const shapeBounds=bounds(shape),legacy=[...elements].reverse().find(el=>{if(el.type!=='text')return false;const textBounds=bounds(el),x=textBounds.x+textBounds.w/2,y=textBounds.y+textBounds.h/2;return x>=shapeBounds.x&&x<=shapeBounds.x+shapeBounds.w&&y>=shapeBounds.y&&y<=shapeBounds.y+shapeBounds.h;});setInlineEditor({kind:'shape',targetId:shape.id,legacyTextId:legacy?.id,value:shape.text||legacy?.text||'',x:shape.x,y:shape.y,w:shape.w,h:shape.h,color:shape.textColor||legacy?.color||shape.color,size:24});setSelectedId(shape.id);return;}
    if(hit?.type==='text'){const textBounds=bounds(hit);setInlineEditor({kind:'text',targetId:hit.id,value:hit.text,x:textBounds.x,y:textBounds.y,w:Math.max(220,textBounds.w+30),h:Math.max(70,textBounds.h+20),color:hit.color,size:hit.size||30});setSelectedId(hit.id);return;}
    setInlineEditor({kind:'text',targetId:null,value:'',x:p.x,y:p.y,w:280,h:90,color,size:30});setSelectedId(null);
  };
  const commitInlineText=currentValue=>{const editor=inlineEditor;if(!editor)return;const value=(currentValue??editor.value).trim();if(editor.kind==='shape'){const next=elements.filter(el=>el.id!==editor.legacyTextId).map(el=>el.id===editor.targetId?{...el,text:value,textColor:editor.color}:el);commit(next);setSelectedId(editor.targetId);}else if(editor.targetId){if(value){commit(elements.map(el=>el.id===editor.targetId?{...el,text:value,color:editor.color}:el));setSelectedId(editor.targetId);}else{commit(removeElementAndConnections(elements,editor.targetId));setSelectedId(null);}}else if(value){const id=crypto.randomUUID();commit([...elements,{id,type:'text',x:editor.x,y:editor.y+(editor.size||30),text:value,color:editor.color,size:editor.size||30,font:'ui'}]);setSelectedId(id);}setInlineEditor(null);};
  ...
  const onPointerUp=()=>{if(!gesture.current)return;const g=gesture.current;gesture.current=null;if(g.kind==='pan'){setIsPanning(false);return;}if(g.kind==='erase'&&!g.changed)return;const next=g.kind==='erase'?g.latest:elements,sliced=history.slice(0,historyIndex+1);persistElements(next);setHistory([...sliced,clone(next)]);setHistoryIndex(sliced.length);if(g.kind==='draw')setSelectedId(g.id);};
  ...
  const inlineEditorStyle=inlineEditor?{left:(inlineEditor.x-camera.x)*zoom,top:(inlineEditor.y-camera.y)*zoom,width:Math.max(inlineEditor.kind==='shape'?100:180,inlineEditor.w*zoom),height:Math.max(inlineEditor.kind==='shape'?54:80,inlineEditor.h*zoom),'--inline-color':inlineEditor.color,'--inline-font-size':`${Math.max(14,(inlineEditor.size||22)*zoom)}px`}:undefined;
  ...
  return <main className="app">
    ...
    <section className={`workspace ${sidebarOpen?'with-sidebar':''}`}>
      ...
      {inlineEditor?<InlineTextEditor editor={inlineEditor} style={inlineEditorStyle} onCommit={commitInlineText} onCancel={()=>setInlineEditor(null)}/>:null}
