...
const loadToolCycle = () => {
  const fallback={mode:'all',whiteboardTools:['pen','eraser','text'],flowchartTools:['rect','diamond','text']};
...
function Whiteboard(){
  ...
  const switchToolMode=mode=>{setToolMode(mode);setTool(mode==='flowchart'?'rect':'select');setColor(mode==='flowchart'?'#17191d':'#175cd3');setSelectedId(null);setConnectorStartId(null);setConnectorDraft(null);setStickerOpen(false);setEraserPoint(null);flash(mode==='flowchart'?'已切换到流程图工具':'已切换到画板工具');};
