...
import supportWechat from './assets/support-wechat.jpg';
...
function DownloadManager({items,loading,isDesktop,onClose,onRefresh,onOpenFolder,onOpen,onReveal}){
  ...
  useEffect(()=>{let active=true;if(!previewItem){setPreviewUrl('');setPreviewLoading(false);return()=>{active=false;};}setPreviewUrl(previewItem.previewUrl||'');if(isDesktop&&window.whiteboardDesktop?.getDownloadedImagePreview){setPreviewLoading(true);window.whiteboardDesktop.getDownloadedImagePreview(previewItem.name,1280).then(url=>{if(active&&url)setPreviewUrl(url);}).catch(()=>{if(active)setPreviewUrl('');}).finally(()=>{if(active)setPreviewLoading(false);});}return()=>{active=false;};},[previewItem,isDesktop]);
  return <div className="modal-scrim" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><div className="app-dialog downloads-dialog" role="dialog" aria-modal="true" aria-labelledby="downloads-title"><div className="dialog-head"><div><h2 id="downloads-title">下载管理</h2><p>直接预览、打开和管理已导出的 PNG 图片</p></div><button className="dialog-close" onClick={onClose} aria-label="关闭"><Icon name="x"/></button></div><div className="download-toolbar"><button className="primary" onClick={onOpenFolder}><Icon name="folder" size={18}/>打开下载目录</button><button className="secondary" onClick={onRefresh}><Icon name="refresh" size={18}/>刷新</button></div>{!isDesktop?<div className="download-browser-note"><Icon name="info" size={18}/><span>浏览器预览显示本次导出的图片；桌面版还可打开图片和定位文件。</span></div>:null}{previewItem?<section className="download-preview" aria-label={`${previewItem.name} 图片预览`}><div className="download-preview-head"><div><strong>{previewItem.name}</strong><small>{formatBytes(previewItem.size)} · {formatDownloadTime(previewItem.updatedAt)}</small></div><button onClick={()=>setPreviewItem(null)} aria-label="关闭图片预览"><Icon name="x" size={18}/></button></div><div className="download-preview-stage">{previewUrl?<img src={previewUrl} alt={`${previewItem.name} 预览`}/>:<div className="download-preview-loading"><Icon name="refresh"/><span>{previewLoading?'正在生成预览…':'暂无可用预览'}</span></div>}</div></section>:null}<div className="download-list">{loading?<div className="download-empty"><Icon name="refresh"/><span>正在读取下载目录…</span></div>:items.length?items.map(item=><article className={`download-item ${previewItem?.name===item.name?'selected':''}`} key={item.path||`${item.name}-${item.updatedAt}`}><button className="download-thumb" onClick={()=>setPreviewItem(item)} aria-label={`预览 ${item.name}`}>{item.previewUrl?<img src={item.previewUrl} alt=""/>:<Icon name="image" size={24}/>}</button><button className="download-info" onClick={()=>setPreviewItem(item)}><strong title={item.name}>{item.name}</strong><small>{formatBytes(item.size)} · {formatDownloadTime(item.updatedAt)}</small></button><div className="download-actions"><button onClick={()=>setPreviewItem(item)}>预览</button><button disabled={!isDesktop||item.browserOnly} onClick={()=>onOpen(item.name)}>打开</button><button disabled={!isDesktop||item.browserOnly} onClick={()=>onReveal(item.name)}>定位</button></div></article>):<div className="download-empty"><Icon name="image"/><strong>还没有导出的图片</strong><span>点击顶部“导出”后，图片会出现在这里。</span></div>}</div></div></div>;
}
...
function Whiteboard(){
  ...
  const loadDownloads=async()=>{setDownloadsLoading(true);try{if(window.whiteboardDesktop?.listDownloadedImages){setDownloads(await window.whiteboardDesktop.listDownloadedImages());}else{const history=JSON.parse(localStorage.getItem(DOWNLOAD_HISTORY_KEY)||'[]');setDownloads(current=>(Array.isArray(history)?history:[]).map(item=>{const live=current.find(entry=>entry.name===item.name);return live?.previewUrl?{...item,previewUrl:live.previewUrl}:item;}));}}catch{flash('读取下载记录失败');}finally{setDownloadsLoading(false);}};
  const openDownloadManager=()=>{setDialog({type:'downloads'});loadDownloads();};
  const rememberBrowserDownload=item=>{try{const current=JSON.parse(localStorage.getItem(DOWNLOAD_HISTORY_KEY)||'[]'),next=[item,...(Array.isArray(current)?current:[]).filter(entry=>entry.name!==item.name)].slice(0,30),persisted=next.map(({previewUrl,...entry})=>entry);localStorage.setItem(DOWNLOAD_HISTORY_KEY,JSON.stringify(persisted));setDownloads(next);}catch{setDownloads(current=>[item,...current].slice(0,30));}};
