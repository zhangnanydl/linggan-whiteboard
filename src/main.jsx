...
const FLOW_TOOLS = [['select','选择'],['connector','自由连线'],['rect','矩形'],['terminator','开始/结束'],['diamond','菱形'],['circle','圆形'],['parallelogram','输入/输出'],['document','文档'],['database','数据库'],['person','小人'],['computer','电脑']];
const TOOLS = [...WHITEBOARD_TOOLS,...FLOW_TOOLS.filter(([id])=>!WHITEBOARD_TOOLS.some(([whiteboardId])=>whiteboardId===id))];
...
const loadToolCycle = () => {
  const fallback={mode:'all',whiteboardTools:['pen','eraser','text'],flowchartTools:['select','rect','connector']};
  try { const value=JSON.parse(localStorage.getItem(TOOL_CYCLE_STORAGE_KEY));if(!value)return fallback;const legacy=Array.isArray(value.tools)?value.tools:[],whiteboardTools=Array.isArray(value.whiteboardTools)?value.whiteboardTools:legacy.filter(id=>WHITEBOARD_TOOLS.some(([toolId])=>toolId===id)),flowchartTools=Array.isArray(value.flowchartTools)?value.flowchartTools:legacy.filter(id=>FLOW_TOOLS.some(([toolId])=>toolId===id));return{mode:value.mode==='custom'?'custom':'all',whiteboardTools:whiteboardTools.length>=3&&whiteboardTools.length<=5?whiteboardTools:fallback.whiteboardTools,flowchartTools:flowchartTools.length>=3&&flowchartTools.length<=5?flowchartTools:fallback.flowchartTools}; } catch { return fallback; }
};
...
function CanvasElement({el,selected=false,thumbnail=false,elementMap,showAnchors=false}){
...
function Whiteboard(){
  ...
  const [isPanning,setIsPanning]=useState(false); const [spacePressed,setSpacePressed]=useState(false); const [hoveringElement,setHoveringElement]=useState(false); const [eraserPoint,setEraserPoint]=useState(null);
  ...
  const onPointerDown=event=>{
    ...
    if(tool==='connector'){
      ...
      if(!source){setSelectedId(null);flash('请从图形四边的连接点开始拖动');return;}
      gesture.current={kind:'connect',fromId:source.element.id,fromSide:source.side,start:source.point};setConnectorDraft({fromId:source.element.id,fromSide:source.side,start:source.point,toPoint:source.point});setSelectedId(source.element.id);return;
    }
  ...
  const chooseTool=id=>{setTool(id);if(toolMode==='flowchart')setColor('#17191d');setEraserPoint(null);setConnectorStartId(null);setConnectorDraft(null);setSelectedId(null);setStickerOpen(id==='sticker'?!stickerOpen:false);};
  const switchToolMode=mode=>{setToolMode(mode);setTool('select');setColor(mode==='flowchart'?'#17191d':'#175cd3');setSelectedId(null);setConnectorStartId(null);setConnectorDraft(null);setStickerOpen(false);setEraserPoint(null);flash(mode==='flowchart'?'已切换到流程图工具':'已切换到画板工具');};
  ...
  const onCanvasWheel=event=>{event.preventDefault();if(!event.deltaY)return;const now=Date.now();if(now-lastWheelSwitch.current<160)return;lastWheelSwitch.current=now;const modeIds=activeTools.map(item=>item[0]),configured=toolMode==='flowchart'?toolCycle.flowchartTools:toolCycle.whiteboardTools,ids=toolCycle.mode==='custom'?configured:modeIds,currentIndex=ids.indexOf(tool),nextIndex=currentIndex<0?(event.deltaY>0?0:ids.length-1):(currentIndex+(event.deltaY>0?1:-1)+ids.length)%ids.length,nextTool=ids[nextIndex],label=activeTools.find(item=>item[0]===nextTool)?.[1]||nextTool;setTool(nextTool);setSelectedId(null);setConnectorStartId(null);setConnectorDraft(null);setStickerOpen(false);setEraserPoint(null);flash(`${toolMode==='flowchart'?'流程图':'画板'}：${label}`);};
  const toggleCycleTool=id=>setSettingsDraft(current=>{if(!current)return current;const key=toolMode==='flowchart'?'flowchartTools':'whiteboardTools',tools=current[key],selected=tools.includes(id);if(selected&&tools.length<=3){flash('自定义轮询至少选择 3 个工具');return current;}if(!selected&&tools.length>=5){flash('自定义轮询最多选择 5 个工具');return current;}const chosen=new Set(selected?tools.filter(toolId=>toolId!==id):[...tools,id]);return{...current,[key]:activeTools.map(item=>item[0]).filter(toolId=>chosen.has(toolId))};});
